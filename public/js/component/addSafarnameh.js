...
function openNewSafarnameh() {
    if (checkLogin()) {
        if ($("#newSafarnamehId").val() != 0) emptySafarnamehModal();
...
function storeSafarnameh() {
    ...
    if (title.trim().length < 2) {
        $("#newSafarnamehError").append(
            "<li>انتخاب عنوان برای سفرنامه الزامی است.</li>"
        );
        error = true;
    }
    if (safarnamehNewMainPic == null && newSafarnamehId == 0) {
        $("#newSafarnamehError").append(
            "<li>انتخاب عکس برای سفرنامه الزامی است.</li>"
        );
        error = true;
    }
    if (text.trim().length < 10) {
        $("#newSafarnamehError").append(
            "<li>نوشتن متن برای سفرنامه الزامی است.</li>"
        );
        error = true;
    }
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response == "ok") {
                    ...
                    showSuccessNotifi(
                        "در ثبت سفرنامه مشکلی پیش امده لطفا دوباره تلاش نمایید.",
                        "left",
                        "red"
                    );
                    closeLoading();
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.status == "ok") {
                ...
                showSuccessNotifi(
                    "در ویرایش سفرنامه مشکلی پیش امده لطفا دوباره تلاش نمایید.",
                    "left",
                    "red"
                );
        },
...
function createEditSafarnameh(_result) {
    ...
    if (_result.tags[0]) $("#safarnamehTag1").val(_result.tags[0]);
    if (_result.tags[1]) $("#safarnamehTag2").val(_result.tags[1]);
    if (_result.tags[2]) $("#safarnamehTag3").val(_result.tags[2]);
...
function createSuggestion(_result) {
    ...
    _result.forEach((item, index) => {
        text +=
            '<div id="place_' +
            item.id +
            '" class="suggEach" onclick="chooseThisSuggestion(' +
            index +
            ')">\n' +
            '    <div class="suggPic">\n' +
            '        <img alt="Dorna News" src="' +
            item.pic +
            '" style="height: 100%">\n' +
            "    </div>\n" +
            '    <div class="suggInfo">\n' +
    ...
    if (_result.length == 0) $(".ourSuggestionShow").hide();
    else $(".ourSuggestionShow").show();
}

function createPickPlace() {
    if (pickedPlaces.length == 0) $("#pickPlacesTitle").hide();
    else $("#pickPlacesTitle").show();
    ...
    pickedPlaces.forEach((item, index) => {
        text +=
            ...
            item.id +
            '" class="suggEach">\n' +
            '    <div class="iconClose deletePickPlace" onclick="deleteFromPickPlace(' +
            index +
            ')"></div>' +
            '    <div class="suggPic">\n' +
            '        <img alt="کوچیتا، سامانه جامع گردشگری ایران" src="' +
            item.pic +
            '" style="height: 100%">\n' +
            "    </div>\n" +
            '    <div class="suggInfo">\n' +
...
function chooseThisSuggestion(_index) {
    ...
    if (!inPick) pickedPlaces.push(sug);
...
function searchForPlaces(_text) {
    if (getSuggestionPlaceAjax != null) getSuggestionPlaceAjax.abort();
