var openGlobalMaterialSearch = () =>
    createSearchInput(
        getGlobalInputMaterialSearchKeyUp,
        "ماده اولبه مورد نظر خود را وارد کنید."
    );

var getGlobalInputMaterialSearchKeyUp = (_element) =>
    searchForMaterial($(_element).val());

var goToCookFestival = () =>
    openLoading(false, () => (location.href = cookFestivalUrl));
...
function lp_selectMenu(id, element) {
    ...
    if (id == "lp_others_myTravel") {
        ...
        getMyTripsPromiseFunc().then((response) =>
            createTripCardFooter(response)
        );
