...
function checkBtnDisable() {
    if ($("#tripNameEdit").val() == "") $("#editBtn").addClass("disabled");
    else $("#editBtn").removeClass("disabled");
}

function sortBaseOnPlaceDate(sortMode) {
    document.location.href =
        tripPlaces + (sortMode === "DESC" ? "/ASC" : "/DESC");
}
...
function deleteTrip() {
    openWarning(
        "آیا از پاک کردن سفر اطمینان دارید ؟",
        doDeleteTrip,
        "بله پاک شود"
    );
}
...
function doDeleteTrip() {
    $.ajax({
        ...
        success: (response) => {
            if (response == "ok") document.location.href = myTripUrlInInner;
        },
    });
}
    ...
    $.ajax({
        ...
        success: (response) => {
            if (response == "ok") {
                ...
                showSuccessNotifi(
                    "یادداشت شما برای سفر با موفقیت ثبت شد",
                    "left",
                    "var(--koochita-blue)"
                );
                $("#tripNotePElement")
                ...
                showSuccessNotifi(
                    "در ثبت یادداشت مشکلی پیش امده لطفا دوباره تلاش نمایید",
                    "left",
                    "red"
                );
        },
...
function chooseSearchMyTrip(_index) {
    ...
    $.ajax({
        ...
        success: (response) => {
            if (response.status == "ok") {
                showSuccessNotifi(
                    "محل مورد نظر به لیست سفر اضافه شد",
                    "left",
                    "var(--koochita-blue)"
                );
                location.reload();
                ...
                if (response.status == "notAccess")
                    ...
                    errorText =
                        "مشکلی در ثبت به وجود امده لطفا دوباره تلاش نمایید";
...
function deletePlace(tripPlaceId) {
    ...
    openWarning(
        "آیا از پاک کردن محل از برنامه سفر خود اطمینان دارید ؟",
        doDeleteTripPlace,
        "بله پاک شود"
    );
}
...
function doDeleteTripPlace() {
    ...
    $.ajax({
        ...
        success: (response) => {
            ...
            if (response.trim() == "ok") {
                showSuccessNotifi(
                    "محل مورد نظر از لیست سفر حذف شد",
                    "left",
                    "var(--koochita-blue)"
                );
...
function assignDateToPlace(tripPlaceId) {
    ...
    if (tripInfo.from_.length != 0) calendarOption.minDate = tripInfo.from_;
    if (tripInfo.to_.length != 0) calendarOption.maxDate = tripInfo.to_;
    ...
    if ($("#date_input").val() != "") {
        $.ajax({
            ...
            success: (response) => {
                if (response === "ok") {
                    showSuccessNotifi(
                        "تاریخ به محل مورد نظر از اضافه شد",
                        "left",
                        "var(--koochita-blue)"
                    );
                    ...
                    closeLoading();
                    showSuccessNotifi(
                        "مشکلی در درخواست شما به وجود امده",
                        "left",
                        "red"
                    );
                    $("#errorText")
                        .empty()
                        .append("تاریخ مورد نظر در بازه ی سفر قرار ندارد");
                }
            },
            error: (err) => {
                closeLoading();
...
function editTrip() {
    ...
    if (
        ...
        newElement =
            "<p class='color-red'>تاریخ پایان از تاریخ شروع باید بزرگ تر باشد</p>";
        $("#error").empty().append(newElement);
        showSuccessNotifi(
            "تاریخ پایان از تاریخ شروع باید بزرگ تر باشد",
            "left",
            "red"
        );
        return;
    ...
    if (tripName.trim().length > 0) {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response == "ok") {
                    showSuccessNotifi(
                        "اطلاعات سفر شما با موفقیت تغییر یافت",
                        "left",
                        "var(--koochita-blue)"
                    );
                    document.location.reload();
                    ...
                    closeLoading();
                    showSuccessNotifi(
                        "در ویرایش اطلاعات سفر مشکلی پیش امده",
                        "left",
                        "red"
                    );
                }
...
function showPlaceInfo(_id, _index) {
    ...
    tripPlacesInfo.forEach((item) => {
        if (item.id == _id) selectedPlace = item;
    });
    ...
    if (selectedPlace != null && openId != _id) {
        ...
        var elemWidth =
            placeElement.width() +
            2 * parseFloat(placeElement.css("padding").split("px")[0]);
        ...
        if (placeInfo.address) $(".address").text(placeInfo.address);
        else if (placeInfo.dastresi) $(".address").text(placeInfo.dastresi);

        $(".reviewCount").text(placeInfo.review + " نقد ");
        $(".rating")
            .find(".ui_bubble_rating")
            .addClass("bubble_" + placeInfo.rate + "0");
        $(".rightSec")
            .find(".city")
            .text(placeInfo.city + " در " + placeInfo.state);

        if (selectedPlace.comments.length == 0)
            $(".userComments").html(
                '<div class="notRow"> برای این محل هیچ یادداشتی ثبت نشده است </div>'
            );
        else createCommentsHtml(selectedPlace);
    } else openId = 0;
}
...
function addComment() {
    ...
    inputs.map((item) => {
        if ($(inputs[item]).val().trim() != "") text = $(inputs[item]).val();
    });
                ...
                if (response.status == "ok") {
                    tripPlacesInfo.forEach((item) => {
                        if (item.id == tripPlaceId) {
                            ...
                            $("#tripCommentNumber" + tripPlaceId)
                                .show()
                                .text(item.comments.length + " یادداشت ");
                        }
                    });
                    showSuccessNotifi(
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response == "ok") {
                tripPlacesInfo.forEach((item) => {
                    ...
                    if (item.id == _placeId) {
                        item.comments.map((comment, _index) => {
                            if (comment.id == _id) index = _index;
                        });
                ...
                showSuccessNotifi(
                    "در حذف یادداشت مشکلی پیش امده لطفا دوباره تلاش نمایید",
                    "left",
                    "red"
                );
        },
...
function choosePlaceInfoTab(_kind, _element) {
    ...
    else placeDetailsToggleBarElement.find(".leftSec").addClass("show");
}
...
function showMembers() {
    ...
    if (tripMember.length == 0)
        $("#members").html(
            `<button class="ui_icon add-friend-fill inviteFBut" onclick="closeMyModal('memberModal'); openMyModal('inviteMember')">دعوت از دوستان</button>`
        );
    else {
        $("#members").empty();
        tripMember.map((item) => {
            ...
            if (item.status == 1) memberElement.find(".loading").remove();
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.status == "ok") {
                ...
                showSuccessNotifi(
                    "تغییر دسترسی با موفقیت اعمال شد",
                    "left",
                    "var(--koochita-blue)"
                );
            } else {
                showSuccessNotifi(
                    "تغییر دسترسی با مشکل مواجه شد",
                    "left",
                    "red"
                );
            }
...
function deleteMember(_id, _username) {
    deletedUserId = _id;
    openWarning(
        "آیا می خواهید " + _username + " را از سفر خود حذف کنید؟",
        doDeleteMember,
        "بله حذف شود"
    );
}
...
function openUserMyTripSearch(_value) {
    ...
    $("#inviteMemberModalSearchButton")
        .find(".iconClose")
        .removeClass("hidden");
}

function closeUserMyTripSearch(_value) {
    if (_value == 0) $("#myTripUserSearchInput").val("");
...
function searchForUserMyTrip(_value) {
    ...
    if (_value.trim().length > 1) {
        ...
        $("#searchResultInviteMember")
            .html(userSearchPlaceHolder + userSearchPlaceHolder)
            .removeClass("hidden");

        searchForUserCommon(_value)
            .then((response) =>
                createInviteMemberSearchResult(response.userName)
            )
    ...
    $.ajax({
        ...
        success: (response) => {
            ...
            if (response.status == "ok") {
                ...
                showSuccessNotifi(
                    "دوست شما با موفقیت به سفر اضافه شد",
                    "left",
                    "var(--koochita-blue)"
                );
                ...
                showSuccessNotifi(
                    "شما دسترسی به دعوت دیگران ندارید",
                    "left",
                    "red"
                );
            else if (response.status == "nok1")
                showSuccessNotifi(
                    "شما نمی توانید خود را دعوت کنید.",
                    "left",
                    "red"
                );
            else if (response.status == "beforeRegistered")
                showSuccessNotifi(
                    "کاربر مورد نظر عضو سفر می باشد",
                    "left",
                    "red"
                );
            else
...
function bubbles(curr) {
    ...
    newElement =
        "<div class='col-xs-12 position-relative'><div class='col-xs-12 bubbles pd-0 mg-rt-0' style='margin-left: " +
        (400 - t * 18) / 2 +
        "px'>";
    ...
    for (i = 1; i < total; i++) {
        if (!isInFilters(i)) {
            if (i == curr)
                ...
                newElement +=
                    "<div class='isNotInFilter helpBubble' onclick='show(\"" +
                    i +
                    "\", 1)'></div>";
        }
    }
...
function show(curr, inc) {
    ...
    var targetWidth = $("#targetHelp_" + curr)
        .css("width")
        .split("px")[0];
