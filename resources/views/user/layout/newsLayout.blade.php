...
<body>
    ...
    <div class="container-fluid secHeadMain hideOnPhone">
	    ...
	    <div class="secHeadTabs">
                <a href="{{route('site.news.main')}}" style="color: #16174f">صفحه اصلی</a>
            </div>
		...
		@if(trim($category->name) != 'استان ها' && $category->name != 'اکوسیستم استارتاپی')
                ...
                <div class="secHeadTabs {{count($category->sub) > 0 ? 'arrowAfter' : ''}}">
                    <a href="{{route('site.news.list', ['kind' => 'category', 'content' => $category->name])}}" style="color: black">{{$category->name}}</a>
            ...
            <div class="newsCategoryListMFooter">
                <div class="list">
                    @foreach($newsCategories as $cat)
                        <a href="{{route('site.news.list', ['kind' => 'category', 'content' => $cat->name])}}" class="categ">
